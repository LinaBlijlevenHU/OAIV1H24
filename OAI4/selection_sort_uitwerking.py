...
def swap(lst, index1, index2):
    ...
    lst[index2] = backup
    # Lijst wordt in geheugen aangepast, dus geen return
...
def find_index_of_minimum(lst, start_index=0):
    ...
    for i in range(start_index, len(lst)):
        # Vraag de waarde van een lijst elementje op
        value = lst[i]

        # Hebben we een nieuw minimum?
        if value < minimum:
            # Overschrijf huidige minimum
            minimum = value
            index_of_minimum = i

    # Geef de positie van het kleinste element terug
...
def selection_sort(lst):
    ...
    for i in range(0, len(lst)-1):
        ...
        min_index = find_index_of_minimum(lst, i)

        # Store it in the current index
